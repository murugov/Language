...
{    
    ...
    ON_DEBUG(
            if (IS_BAD_PTR(arr_cmd))
                return ASM_BAD_ARR_CMD_PTR;
    )
        
    byte_t *code = (byte_t*)calloc((size_t)count_cmd * (1 + sizeof(arg_t)) + 1, sizeof(byte_t));
    if (IS_BAD_PTR(code))
        return ASM_BAD_CODE_PTR;

    size_t count_labels = 0;
    label_t *arr_labels = (label_t*)calloc((size_t)count_cmd, sizeof(label_t));
    if (IS_BAD_PTR(arr_labels))
        return ASM_BAD_ARR_LABELS_PTR;
